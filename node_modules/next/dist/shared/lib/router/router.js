...
class Router {
   ...
   */ replace(url, as, options = {}) {
    ...
    async change(method, url, as, options, forcedScroll) {
        ...
        pathname = pathname ? (0, _removeTrailingSlash).removeTrailingSlash(delBasePath(pathname)) : pathname;
     ...
     */ if ((!options.shallow || options._h === 1) && (options._h !== 1 || (0, _isDynamic).isDynamicRoute((0, _removeTrailingSlash).removeTrailingSlash(pathname)))) {
   ...
   */ async prefetch(url, asPath = url, options = {}) {
    ...
    async _preflightRequest(options) {
        ...
        if (preflight.rewrite) {
            ...
            const fsPathname = (0, _removeTrailingSlash).removeTrailingSlash(parsed.pathname);
            let matchedPage;
            let resolvedHref;
        ...
        if (preflight.redirect) {
            if (preflight.redirect.startsWith('/')) {
                const cleanRedirect = (0, _removeTrailingSlash).removeTrailingSlash((0, _normalizeLocalePath).normalizeLocalePath(hasBasePath(preflight.redirect) ? delBasePath(preflight.redirect) : preflight.redirect, this.locales).pathname);
